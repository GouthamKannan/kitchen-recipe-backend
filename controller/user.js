...
const getOneUserCode = async ( ver_code ) => {
  const response = await UserModel.find({ ver_code});
  return response
};
...
const getOneUserName = async ( user_name, email ) => {
  const response = await UserModel.find({ user_name, email });
  return response
};

const getOneUserId = async ( id ) => {
  const response = await UserModel.find({ _id : new mongodb.ObjectId(id) });
  return response
};
...
const getUserResetCode = async(reset_code) => {
  const response = await codeModel.find({reset_code})
  return response;
}
...
const reset_password = async(password, reset_code) => {
 ...
 if (response.length == 1) {
     ...
     await codeModel.deleteOne({email : response[0].email})
     return true
...
const send_reset_link = async(email, protocol, host) => {
 ...
 if(sent==true) {
   ...
   await codeModel.collection.insertOne({
     createdAt: new Date(),
     email,
     reset_code
   })
